...
const updateReservedHour = async (reservedhours: ReservedHours) => {
    await dbQuery(`UPDATE reservedhours SET id_company = ?, id_client = ?, from_hour = ?, to_hour = ?, week_day = ? WHERE id_rhours = ?`,
    [reservedhours.id_company, reservedhours.id_client, reservedhours.from_hour, reservedhours.to_hour, reservedhours.week_day, reservedhours.id_rhours]);
